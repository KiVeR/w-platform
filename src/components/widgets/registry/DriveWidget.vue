...
<template>
  ...
  >
    ...
    <div v-if="hasStores && stores[0]" class="drive-preview">
      <div class="preview-title">Magasin le plus proche :</div>
      <div class="store-card">
        <div class="store-info">
          <div class="store-name">{{ stores[0]?.name }}</div>
          <div class="store-address">{{ stores[0]?.address }}</div>
    ...
    <div v-else class="drive-empty">
      <p class="empty-text">Aucun magasin configuré</p>
      <p class="empty-hint">Ajoutez des magasins dans les options pour activer la géolocalisation</p>
    </div>
  </div>
</template>
