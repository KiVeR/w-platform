...
it('allows admin to mark all fields as used', function (): void {
    $admin = User::factory()->create();
    $admin->assignRole('admin');
    Passport::actingAs($admin);

    $schema = VariableSchema::factory()->create();
    VariableField::factory()->forSchema($schema)->count(3)->create(['is_used' => false]);

    $response = $this->postJson("/api/variable-schemas/{$schema->uuid}/mark-used");

    $response->assertOk();

    $allUsed = VariableField::where('variable_schema_id', $schema->id)
        ->where('is_used', false)
        ->count();
    expect($allUsed)->toBe(0);
});

it('allows partner to mark used on own schema', function (): void {
    $partner = Partner::factory()->create();
    $user = User::factory()->forPartner($partner)->create();
    $user->assignRole('partner');
    Passport::actingAs($user);
    ...
    $schema = VariableSchema::factory()->forPartner($partner)->create();
    VariableField::factory()->forSchema($schema)->count(2)->create(['is_used' => false]);
    ...
    expect(
        VariableField::where('variable_schema_id', $schema->id)
            ->where('is_used', true)
            ->count()
    )->toBe(2);
});
...
it('denies partner from marking used on other partner schema', function (): void {
    ...
    $this->postJson("/api/variable-schemas/{$schema->uuid}/mark-used")->assertForbidden();
});

it('allows admin to mark all fields as unused', function (): void {
    $admin = User::factory()->create();
    $admin->assignRole('admin');
    Passport::actingAs($admin);

    $schema = VariableSchema::factory()->create();
    VariableField::factory()->forSchema($schema)->count(3)->create(['is_used' => true]);
...
it('allows partner to mark unused on own schema', function (): void {
    $partner = Partner::factory()->create();
    $user = User::factory()->forPartner($partner)->create();
    $user->assignRole('partner');
    Passport::actingAs($user);
    ...
    $schema = VariableSchema::factory()->forPartner($partner)->create();
    VariableField::factory()->forSchema($schema)->count(2)->create(['is_used' => true]);
...
it('denies partner from marking unused on other partner schema', function (): void {
    ...
    $this->postJson("/api/variable-schemas/{$schema->uuid}/mark-unused")->assertForbidden();
});

// ========================================================================
// DISCOVER / PREVIEW (stub — 501)
// ========================================================================

it('returns 501 for discover endpoint', function (): void {
    $admin = User::factory()->create();
    $admin->assignRole('admin');
    Passport::actingAs($admin);

    $this->postJson('/api/variable-schemas/discover')
        ->assertStatus(501)
        ->assertJsonPath('message', 'Not implemented.');
});

it('returns 501 for preview endpoint', function (): void {
    $admin = User::factory()->create();
    $admin->assignRole('admin');
    Passport::actingAs($admin);

    $this->postJson('/api/variable-schemas/preview')
        ->assertStatus(501)
        ->assertJsonPath('message', 'Not implemented.');
});
