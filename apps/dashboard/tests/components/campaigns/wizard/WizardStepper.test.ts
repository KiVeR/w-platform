...
const fakeSteps: WizardStep[] = [
  { key: 'estimate', labelKey: 'wizard.steps.estimate', icon: DummyIcon },
...
describe('WizardStepper', () => {
  it('render toutes les étapes avec labels', () => {
    ...
    expect(text).toContain('wizard.steps.estimate')
  ...
  it('step completed + valide → cercle success', () => {
    ...
    const step1 = wrapper.find('[data-step="type"]')
    expect(step1.attributes('data-step-valid')).toBe('true')
  ...
  it('divider entre 2 steps valides → success', () => {
    const wrapper = mountStepper(4, [true, true, true, true, false, false])

    const dividers = wrapper.findAll('[data-divider]')
    // divider index 2 = between step 2 and step 3 (both completed & valid)
    expect(dividers[2].classes()).toContain('bg-success-500')
  })
  ...
  it('divider entre steps mixtes → warning', () => {
    ...
    const dividers = wrapper.findAll('[data-divider]')
    // divider index 1 = between step 0 (valid) and step 1 (invalid, completed)
  ...
  it('step 0 (estimate) cliquable depuis n\'importe quel step', async () => {
    const wrapper = mountStepper(4, [true, true, true, true, false, false])

    const step0 = wrapper.find('[data-step="estimate"]')
  ...
  it('step 0 (estimate) pas grisé quand currentStep > 0', () => {
    const wrapper = mountStepper(3, [true, true, true, false, false, false])

    const step0 = wrapper.find('[data-step="estimate"]')
    expect(step0.classes()).toContain('step-completed')
    expect(step0.classes()).not.toContain('step-future')
  })
})
